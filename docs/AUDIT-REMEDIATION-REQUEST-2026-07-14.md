...
- 구현 기준선: `OpenSphere-console@3ff532f`, `OpenSphere-Setup-CLI@bccae34`
- 대상 채널: `edge` (개발·검증 채널)
...
| 증거 | 결과 | 확인 범위 |
|---|---|---|
| Setup 단위·계약 테스트 | 61/61 통과 | provenance/SBOM, lock 무결성, 권한·백업·TLS·service credential 게이트, multi-arch, port-forward |
| GitHub Actions run `29324393980` | 성공 | 새 kind 클러스터에서 edge clean bootstrap, provenance/SBOM, PostgreSQL 권한 경계, backup/restore drill, `os` 설치 |
| GitHub Actions run `29324952556` | 성공 | development 및 production 인증 프로파일의 edge clean bootstrap; 과거 서명 lock → 현재 lock upgrade → 과거 lock rollback |
| rollback run의 라이브 결과 | 성공 | 14 Pod / 12 Service, runtime image digest lock, 세 단계 각각 PostgreSQL backup 및 비파괴 restore drill 완료 |

검증 실행 링크: <https://github.com/opensphere-platform/OpenSphere-Setup-CLI/actions/runs/29324952556>

## 3. 감사 지적별 시정 상태

| 감사 ID | 시정 상태 | 재감사에서 확인할 핵심 |
|---|---|---|
| `INT-AUD-001` P0 | 구현 및 clean-cluster 라이브 검증 완료 | 런타임 `console` role의 superuser/소유권/UPDATE·DELETE·TRUNCATE·trigger 우회 불가를 직접 질의 |
| `INT-AUD-002` P1 | 구현 및 CI 검증 완료 | 9개 이미지 모두 OCI-bound provenance와 SPDX SBOM을 fail-closed로 검증하는지 확인 |
...
| `INT-AUD-005` P1 | 구현·clean kind 검증 완료 | Service port-forward 기반 verify와 `linux/amd64`, `linux/arm64` 검증; 별도 원격 지원 K8s에서 독립 재현 권고 |
