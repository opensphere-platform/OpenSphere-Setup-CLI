...
2026-09-23 관리자 결정에 따라 이후 발행은 실제 사용하는 자산만 만든다.
현재 대상은 RKE2 Linux amd64이므로 `opensphere-setup-linux-amd64.tar.gz`와
`SHA256SUMS`만 발행한다. 다른 OS/CPU는 실제 설치 요구가 생길 때 별도로
검토하여 발행 대상에 추가한다. 아래 표와 플랫폼별 명령은 이미 발행한
edge.34의 기능을 설명하며, 이후 버전에도 모든 자산이 있다는 의미가 아니다.
