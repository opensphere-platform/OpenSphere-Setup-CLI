...
function cleanBootstrapCredentialBlock(source) {
  const anchor = 'password: hex(32),\n      bootstrap_password: hex(32),';
  const start = source.indexOf(anchor);
  assert.ok(start >= 0, 'clean bootstrap must declare password and bootstrap_password together as the backbone-postgres literal object');
...
test('oaa_password is never assigned by reusing the password or bootstrap_password expression/value', async () => {
  const source = await readFile(join(ROOT, 'src', 'bootstrap.mjs'), 'utf8');
  const body = cleanBootstrapCredentialBlock(source);
  // Each of the three credential keys must be its own independent hex(32) call
  // (a fresh randomBytes draw), not a shared variable or a copy of another key.
  const calls = body.match(/hex\(32\)/g) || [];
  assert.equal(calls.length, 3, 'password, bootstrap_password and oaa_password must each call hex(32) independently');
