...
function preparedHarness({failEstablishment=false,failHiss=false,failCore=false,failCeph=false}={}) {
  ...
  const fn=vm.runInNewContext('('+source.slice(preparedBegin,preparedEnd)+')',{
    ...
    prepareCephExecutionProfile:()=>{assert.equal(state.core,true);if(failCeph)throw Error('Ceph execution profile conflict');state.ceph=true;},
...
test('HISS authority preparation is awaited and a conflict prevents later Main Shell deployment',async()=>{
  ...
  assert.equal(ok.state.ceph,true);
  const cephFailure=preparedHarness({failCeph:true});await assert.rejects(cephFailure.run(prepared),/Ceph execution profile conflict/);
  assert.ok(!cephFailure.state.applied.includes('deploy/opensphere-console.yaml'));
