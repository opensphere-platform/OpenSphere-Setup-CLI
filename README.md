...
이전 관리 목록이 아예 없다면 명시적 복구는 해당 릴리스의 공식 manifest에서 Setup 관리 대상을 재구성한다. 목록 조회 거절이나 손상은 부재로 취급하지 않으며 namespace 전체 자원을 임의로 편입하지 않는다. TTL로 Beszel 초기 구성 Job과 기존 완료 증거가 모두 사라진 설치에서는 기존 Secret·서비스 계정을 사용하는 공식 bootstrap Job을 재실행해 증거를 복원한다. 기존 Job이 실패했거나 다른 이미지이면 자동 삭제·교체하지 않는다. 일반 업그레이드의 검증·롤백 정책은 바꾸지 않는다.
